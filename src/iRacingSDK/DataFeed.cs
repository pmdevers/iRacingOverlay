...
namespace iRacingSDK
{
	internal class DataFeed
	{
		...
		public unsafe DataSample GetNextDataSample(int requestedTickCount, bool logging)
		{
			...
			{
				Telemetry = variables, SessionData = sessionData, Descriptions = variableDescriptions, IsConnected = true
			};
		}

		unsafe iRSDKHeader ReadHeader(byte* ptr)
		{
			return (iRSDKHeader) Marshal.PtrToStructure(new IntPtr(ptr), typeof(iRSDKHeader));
		}
        ...
        private DataSample DisconnectedSample()
		{
        ...
        private SessionData ReadSessionInfo(iRSDKHeader header)
		{
			if (header.sessionInfoUpdate == _sessionLastInfoUpdate)
				return _lastSessionInfo;

			_sessionLastInfoUpdate = header.sessionInfoUpdate;
			Trace.WriteLine($"New Session data retrieved from iRacing. {_sessionLastInfoUpdate}", "DEBUG");
